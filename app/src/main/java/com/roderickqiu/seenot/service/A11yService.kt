...
class A11yService : AccessibilityService() {
                    ...
                    try {
                        ...
                        if (result != null) {
                            ...
                            if (isConditionMatch && rule.timeConstraint != null) {
                                handleTimeConstraint(rule, appName, isConditionMatch)
    ...
    private fun endShortTermRecord(ruleId: String, appName: String) {
     ...
     */
    private fun loadTimeConstraintStates() {
        val now = System.currentTimeMillis()
    ...
    private fun triggerAction(rule: Rule, appName: String) {
        when (rule.action.type) {
            ActionType.REMIND -> {
                ...
                showToast("$appName: $message", Toast.LENGTH_LONG)
