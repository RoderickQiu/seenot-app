...
object GuardedModePrefs {
    ...
    fun isDimmingEnabled(context: Context): Boolean =
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE).getBoolean(DIMMING, true)
    fun setDimmingEnabled(context: Context, enabled: Boolean) {
        context.getSharedPreferences(NAME, Context.MODE_PRIVATE).edit().putBoolean(DIMMING, enabled).apply()
    }
}
